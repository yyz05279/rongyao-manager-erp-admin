...
export class ExcelTemplateGenerator {
  ...
  static generateTemplate(config: any): void {
    ...
    // 添加表头
    const headers = config.columns.map((col: any) => 
  ...
  private static createInstructionSheet(config: any): XLSX.WorkSheet {
    ...
    instructionData.push(['1. 设备类型请填写：MECHANICAL(机械设备)、ELECTRICAL(电控设备)、PIPELINE(管路设备)、BURNER(燃烧器)、AUXILIARY(辅助设备)、STANDARD_PARTS(标准件)']);
    ...
    // 设置列宽
    worksheet['!cols'] = [
      { wch: 20 }, { wch: 10 }, { wch: 10 }, { wch: 20 }, { wch: 40 }
    ];
