<template>
  ...
  >
    ...
    >
      ...
      <el-card class="form-card" shadow="never">
        <template #header>
          <span class="card-title">🔬 NaNO2 (亚硝酸钠) 配比信息</span>
        </template>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="目标配比(%)" prop="nano2TargetRatio">
              <el-input-number
                v-model="formData.nano2TargetRatio"
                :precision="2"
                :min="0"
                :max="100"
                style="width: 100%"
          ...
          <el-col :span="12">
            <el-form-item label="实际配比(%)" prop="nano2ActualRatio">
              <el-input-number
                v-model="formData.nano2ActualRatio"
                :precision="2"
                :min="0"
                :max="100"
                style="width: 100%"
          ...
          <el-col :span="12">
            <el-form-item label="目标用量(kg)" prop="nano2TargetWeight">
              <el-input-number
                v-model="formData.nano2TargetWeight"
                :precision="2"
                :min="0"
                style="width: 100%"
              />
          ...
          <el-col :span="12">
            <el-form-item label="实际用量(kg)" prop="nano2ActualWeight">
              <el-input-number
                v-model="formData.nano2ActualWeight"
                :precision="2"
                :min="0"
                style="width: 100%"
              />
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="稳定性指数" prop="stabilityIndex">
              <el-input-number
                v-model="formData.stabilityIndex"
                :precision="2"
                :min="0"
                :max="10"
                style="width: 100%"
...
import { getTernaryRecord, addTernaryRecord, updateTernaryRecord } from '@/api/erp/saltprocess/records/ternary';
...
const getRecordDetail = async () => {
  ...
  loading.value = true;
  try {
    const { data } = await getTernaryRecord(props.recordId);
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await updateTernaryRecord({ ...formData, id: props.recordId });
      ElMessage.success('更新成功');
    } else {
      await addTernaryRecord(formData);
