...
**文件**：`src/utils/request.ts` (第58-64行)
...
**流程**：
1. 检测到GET请求且有 `params` 对象
2. 调用 `tansParams` 函数将 `params` 转换为URL查询字符串
3. 清空 `params` 对象，将完整URL赋值给 `config.url`

---

### 2. tansParams 函数实现

**文件**：`src/utils/ruoyi.ts` (第210-230行)
...
**问题点**：
- 第216行：`if (typeof value === 'object')`
- 如果 `templateId` 的值是对象类型（包括数组），就会进入对象处理分支
- 第219行：`propName + '[' + key + ']'` 会产生类似 `templateId[0]` 的格式
...
**文件**：`src/utils/request.ts` (第54-56行)
...
**ID转换器逻辑**：`src/utils/id-converter.ts`
...
**分析**：
- `convertRequestIds` 会递归处理对象，将ID字段转换为字符串
...
2. **绕过拦截器处理**：
   - axios拦截器第58行：`if (config.method === 'get' && config.params)`
...
1. ❌ axios的 `params` 对象可能被某些处理逻辑错误地转换为数组格式
2. ❌ `tansParams` 函数对对象类型的值使用 `propName[key]` 格式
3. ❌ 后端Spring Boot无法将数组转换为单个Long值

### 解决方案

✅ **直接在URL中拼接查询参数，完全绕过 `params` 对象的序列化过程**
