...
4. **批量分配工具**
   ...
   assignShippingTypes(items, defaultShippingType): 明细列表
   ```
   - 为明细数据批量分配默认值
   - 明细级别的值优先于主表级别
   - 三个工具函数分别处理设备类型、子系统、发货类型
...
2. ✅ 标题不包含括号：`机械设备发货清单`
3. ✅ 标题无类型信息：`发货清单`
4. ✅ 第二行带前缀：`一、固态处理厂`、`1、固态处理厂`
5. ✅ 第二行不带前缀：`固态处理厂`
6. ✅ 第二行为对象：`{ subsystem: '固态处理厂' }`
7. ✅ 第二行为空：`null` 或 `undefined`
8. ✅ 明细覆盖主表：部分明细独立设置
9. ✅ 多 Sheet 场景：每个 Sheet 不同元数据
10. ✅ shippingType 字段：主表和明细都正确填充
...
2. **批量处理**: 使用 `assignEquipmentTypes`、`assignSubsystems`、`assignShippingTypes` 批量分配
3. **类型安全**: 始终使用 TypeScript 类型定义
4. **错误处理**: 对提取失败的情况做好容错处理
5. **用户反馈**: 在 UI 中展示提取的元数据，让用户确认
6. **字段一致性**: 确保主表和明细的 `shippingType`、`subsystem` 字段一致填充
